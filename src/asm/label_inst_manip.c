...
/*   Updated: 2016/03/12 19:07:12 by rporcon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_add_instruc(char *buf, t_label *lbl_lst)
{
	...
	if (is_in_str(SEPARATOR_CHAR, trim) == 1)
	{
		...
		new_elem->param_2 = inst_line[1];
		addend_inst_lst(&inst_lst, new_elem);
		if (tmp_lbl)
			tmp_lbl->insts = new_elem;
		return (1);
	}
	else if (is_in_str(SEPARATOR_CHAR, trim) == 2)
	{
		// three params
		ft_putstr("LOL");
		return (1);
	}
	// need to check only one param
	return (0);
}
